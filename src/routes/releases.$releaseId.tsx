...
import { DemoBanner, EmptyState, KeyValue, PageHeader, Panel, StatusPill, Tag } from "@/components/primitives";
...
export const Route = createFileRoute("/releases/$releaseId")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Release record with every mandatory readiness check and the governing approval." },
...
function ReleaseDetail() {
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-4 p-4 lg:p-6">
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        <Panel title="Release record">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <KeyValue label="State" value={<StatusPill map={RELEASE_STATE} value={release.state} />} />
        ...
        <Panel title="Approval record">
          {approvals.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              No approval recorded. This release cannot be marked ready or shipped without an explicit founder approval record.
            </p>
          ) : (
            approvals.map((a) => (
              <div key={a.id} className="rounded-xs border border-border bg-canvas p-2 text-[0.8125rem]">
                ...
                <Tag className="mt-1.5" tone={a.decision === "approved" ? "success" : a.decision === "rejected" ? "critical" : "warning"}>
                  {a.decision}
                </Tag>
              </div>
            ))
      ...
      <Panel title="Readiness checks" subtitle="Mandatory checks must be explicitly evaluated — never inferred." dense>
        <DataTable
          rows={checks}
          columns={[
            { key: "label", header: "Check", render: (c) => c.label },
            { key: "mandatory", header: "Mandatory", render: (c) => <Tag tone={c.mandatory ? "warning" : "muted"}>{c.mandatory ? "Required" : "Optional"}</Tag> },
            { key: "state", header: "State", render: (c) => <StatusPill map={GATE_STATE} value={c.state} /> },
