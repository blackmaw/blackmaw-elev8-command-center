  ...
  head: () => ({
    meta: [
      { title: "System Settings — Elev8 Command Center" },
      {
        name: "description",
        content:
          "Environment, appearance, workspace defaults, security posture, and integration state.",
      },
      { property: "og:title", content: "System Settings — Elev8 Command Center" },
      {
        property: "og:description",
        content: "Command center configuration and integration register.",
      },
    ],
  }),
...
function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-4 p-4 lg:p-6">
            ...
            <label className="flex items-center justify-between gap-3 rounded-xs border border-border bg-canvas px-2 py-2 text-[0.8125rem]">
              <span>
                ...
                <span className="block text-xs text-muted-foreground">
                  Play the initialization sequence once per session.
                </span>
              </span>
              <input
                type="checkbox"
                checked={bootEnabled}
                onChange={(e) => setBootEnabled(e.target.checked)}
                className="size-4 accent-[var(--teal-bright,#42A5A5)]"
              />
            </label>
            <label className="flex items-center justify-between gap-3 rounded-xs border border-border bg-canvas px-2 py-2 text-[0.8125rem]">
              <span>
            ...
            <DataTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  header: "Value",
                  render: (s) =>
                    typeof s.value === "boolean" ? (s.value ? "Enabled" : "Disabled") : s.value,
                },
                {
                  key: "editable",
                  header: "Control",
                  render: (s) => (
                    <Tag tone={s.editable ? "teal" : "muted"}>
                      {s.editable ? "Adjustable" : "Fixed"}
                    </Tag>
                  ),
                },
              ]}
      ...
      <Panel
        title="Integrations"
        subtitle="No adapter is connected in this phase — every recorded value is entered by a person."
        dense
      >
        <DataTable
          rows={integrations}
          columns={[
            { key: "name", header: "Integration", render: (i) => i.name },
            {
              key: "category",
              header: "Category",
              render: (i) => i.category.replace(/_/g, " "),
              secondary: true,
            },
