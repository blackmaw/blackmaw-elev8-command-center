...
export const Route = createFileRoute("/notifications")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Approval requests, escalations, and recorded system notices across every workspace." },
        ...
        {(["all", "unread"] as const).map((f) => (
          <button
            ...
            className={`rounded-xs border px-2 py-1 text-[0.75rem] transition-colors ${
              f === filter ? "border-border-strong bg-panel-elevated text-foreground" : "border-border text-muted-foreground hover:text-foreground"
            }`}
          >
            {f === "all" ? "All" : "Unread"}
          </button>
        ...
        <ul className="divide-y divide-border">
          {rows.map((n) => (
            <li key={n.id} className="flex min-w-0 gap-3 px-4 py-3">
              <span className={`mt-1.5 size-1.5 shrink-0 rounded-full ${n.read ? "bg-muted-foreground" : "bg-teal-bright"}`} aria-hidden />
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <span className="text-[0.8125rem] font-medium">{n.title}</span>
                  <Mono className="text-[0.6875rem] text-muted-foreground">{new Date(n.occurred_at).toLocaleString()}</Mono>
                </div>
