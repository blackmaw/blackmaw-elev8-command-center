...
import { DemoBanner, MetricTile, PageHeader, Panel, StatusPill, Tag } from "@/components/primitives";
...
export const Route = createFileRoute("/infrastructure/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Recorded infrastructure: compute, network, storage, topology, and integration state." },
...
function InfrastructureIndex() {
  ...
  const planned = assets.filter((a) => a.lifecycle === "planned" || a.lifecycle === "staging").length;
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-4 p-4 lg:p-6">
      <PageHeader
        ...
        actions={
          <Link to="/infrastructure/topology" className="rounded-xs border border-border px-2 py-1 text-[0.75rem] hover:border-border-strong">
            Full topology
          </Link>
        }
      >
      ...
      <Panel title="Topology" subtitle="Recorded connectivity — select a node for its asset record." dense bodyClassName="p-0">
        <InfraTopology />
      </Panel>

      <Panel title="Asset registry" dense actions={<Link to="/assets" className="text-[0.75rem] text-teal-bright hover:underline">Full inventory</Link>}>
        <DataTable
          rows={assets}
          columns={[
        ...
        <Panel title="Infrastructure risks" dense>
          <DataTable
            ...
            columns={[
              ...
              { key: "state", header: "State", render: (r) => <Tag>{r.state}</Tag>, secondary: true },
            ]}
          />
        </Panel>
      </div>
