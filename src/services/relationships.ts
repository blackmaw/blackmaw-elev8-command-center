...
export function getProductGraph(productId: ID | undefined): ProductGraph | null {
  ...
  const assetIds = new Set(
    db.assets
      .filter((a) => a.services.some((s) => s.toLowerCase().includes(product.name.toLowerCase())))
      .map((a) => a.id),
  );
  ...
  return {
    ...
    releases,
    documents,
    roadmaps: db.roadmaps.filter(
      (r) =>
        r.product_id === product.id ||
        r.lanes.some((l) => l.items.some((i) => i.product_id === product.id)),
    ),
    ...
    journal: db.builderJournal
      .filter((j) => j.product_id === product.id)
      .sort((a, b) => b.entry_date.localeCompare(a.entry_date)),
    activity: db.activityEvents
...
export const approvalsForSubject = (subjectId: ID) =>
  db.approvals.filter((a) => a.subject_id === subjectId);
export const journalForRepository = (repositoryId: ID) =>
  db.builderJournal.filter((j) => j.repository_id === repositoryId);
export const journalForDocument = (documentId: ID) =>
  db.builderJournal.filter((j) => j.document_ids.includes(documentId));
