...
function score(key: string, label: string, value: number, basis: string, factors: { label: string; value: string }[]): HealthScore {
...
export function productHealth(productId: ID): HealthScore {
  ...
  const risks = db.risks.filter((r) => r.product_id === product.id && (r.state === "open" || r.state === "mitigating"));
  ...
  const gates = [product.security_state, product.validation_state].filter((g) => g === "passed").length;
  const value = Math.max(0, Math.min(100, Math.round(product.progress * 0.4 + acceptance * 0.3 + gates * 15 - criticals * 12)));
  return score("product." + product.key, product.name, value, "Progress, acceptance criteria met, gate states, and open risk severity.", [
...
export function repositoryHealth(): HealthScore {
  ...
  const value = Math.round((pct(clean, repos.length) + pct(ci, repos.length) + pct(docs, repos.length)) / 3);
  return score("repository", "Repository health", value, "Working-tree cleanliness, recorded CI state, and documentation review state across tracked repositories.", [
...
export function infrastructureHealth(): HealthScore {
  ...
  const risks = db.risks.filter((r) => r.workspace_id === "infrastructure" && (r.state === "open" || r.state === "mitigating"));
  const critical = risks.filter((r) => r.severity === "critical").length;
  const value = Math.max(0, Math.round(pct(inService, assets.length) - critical * 15 - (risks.length - critical) * 5));
  return score("infrastructure", "Infrastructure health", value, "Assets in service versus registered assets, reduced by open infrastructure risks.", [
...
export function documentationHealth(): HealthScore {
  ...
  return score("documentation", "Documentation health", value, "Approved documents as a share of the register, reduced by documents never evaluated.", [
...
export function releaseHealth(): HealthScore {
  ...
  return score("release", "Release health", value, "Mandatory release checks passed across all recorded releases, reduced by blocked releases.", [
...
export function engineeringHealth(): HealthScore {
  ...
  return score("engineering", "Engineering health", value, "Stages approved or frozen, reduced by blocked stages and pending approvals.", [
...
export function healthBoard(): HealthScore[] {
  return [engineeringHealth(), repositoryHealth(), infrastructureHealth(), documentationHealth(), releaseHealth()];
}
