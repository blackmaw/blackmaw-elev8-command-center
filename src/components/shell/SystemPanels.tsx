...
function Trigger({ children, className, label }: { children: ReactNode; className?: string; label: string }) {
  return (
    <DialogTrigger
      aria-label={label}
      className={cn(
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            This environment runs on a demonstration record set. No production system, repository, or financial account is connected.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <KeyValue label="Last sync" value={new Date(SYSTEM_STATE.last_sync_at).toLocaleString()} />
...
export function SystemStatusControl() {
  ...
  return (
    <Dialog>
      <Trigger label="Open system status" className="justify-end text-[0.6875rem] text-muted-foreground">
        <span className="size-1.5 rounded-full bg-success" aria-hidden />
        {SYSTEM_STATE.system_status}
      </Trigger>
      <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-[0.9375rem]">System status — {SYSTEM_STATE.system_status}</DialogTitle>
          <DialogDescription className="text-xs">
            Shell health is derived from the recorded register. No live probe or telemetry agent is running.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          <KeyValue label="Shell" value={<Tag tone="success">Operational</Tag>} />
          <KeyValue label="Record set" value={<Tag tone="success">Loaded</Tag>} />
          <KeyValue label="Pending approvals" value={<span className="num">{metrics.pendingApprovals}</span>} />
          <KeyValue label="Open blockers" value={<span className="num">{metrics.openBlockers}</span>} />
        </div>
        <div className="space-y-1.5">
          <Label>Integration surface</Label>
          {integrations.map((i) => (
            <div key={i.id} className="flex items-center justify-between gap-3 border-b border-border/60 pb-1 text-[0.8125rem] last:border-0">
...
export function OperatingModeControl() {
  ...
  return (
    <Dialog>
      <Trigger label="Open operating mode details" className="justify-end truncate text-[0.6875rem] text-muted-foreground">
      ...
      <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            The operating mode follows the signed-in role register. Mode selection is governed and fixed while authentication is not connected.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <KeyValue label="Mode changes" value={<Tag tone="muted">Requires connected authentication</Tag>} />
        </div>
        <div className="space-y-1.5">
          <Label>Available modes</Label>
          {Object.values(ROLES).map((r) => (
            <div key={r.key} className="flex items-start justify-between gap-3 border-b border-border/60 pb-1 text-[0.8125rem] last:border-0">
              <span className="min-w-0">
                <span className="block">{r.name}</span>
                <span className="block text-xs text-muted-foreground">{r.description}</span>
              </span>
              <Tag tone={r.key === currentUser.role ? "teal" : "muted"}>{r.key === currentUser.role ? "Active" : "Unavailable"}</Tag>
            </div>
          ))}
        </div>
        <Link to="/settings" className="text-[0.75rem] text-teal-bright hover:underline">
